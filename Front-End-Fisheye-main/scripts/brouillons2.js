...
openLightboxForMedia(index, photographer) {
  ...
  htmlMediaFactory(photographer, media, index) {
    if (media.image) {
      return `
        ...
        <div class="info">
          <p>${media.title}</p>
          <p class="likes" id="likes-${index}">${media.likes} <button class="heart-button" onclick="handleLikeClick(${index}, event)"><span><img src="assets/icons/coeur.svg" alt="coeur"></span></button></p>
        `;
    } else if (media.video) {
      return `
  ...
  setupMediaEventHandlers() {
    ...
    likeButton.addEventListener("click", (event) => {
      ...
      this.handleLikeClick(index, likeButton);
    });
  ...
  handleLikeClick(index, likeButton) {
    ...
    // Incrémente simplement le compteur de likes de 1
    media.likes++;
  
    // Mettez à jour l'affichage du nombre de likes pour ce média
